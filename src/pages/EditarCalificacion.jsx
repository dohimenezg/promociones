...
export default function EditarCalificacion() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await db.calificacionFinanciera.update(id, {
        nombre: data.nombre,
        descripcion: data.descripcion
      });
      navigate(returnUrl, { state: { tab: 'calificacion', id: id } });
    } catch (err) {
      console.error(err);
      ...
      <div className="card" style={{ padding: '2rem' }}>
        ...
        <form onSubmit={handleUpdate}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '3rem' }}>
            <button type="button" className="btn-secondary" onClick={() => navigate(returnUrl, { state: { tab: 'calificacion', id: id } })}>Cancelar</button>
