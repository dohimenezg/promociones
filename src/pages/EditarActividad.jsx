...
export default function EditarActividad() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await db.actividadEconomica.update(id, {
        nombre: data.nombre,
        descripcion: data.descripcion
      });
      navigate(returnUrl, { state: { tab: 'actividad', id: id } });
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '2rem' }}>
      <button 
        onClick={() => navigate(returnUrl, { state: { tab: 'actividad' } })}
      ...
      <h1 className="page-title" style={{ marginBottom: '2rem', color: '#000' }}>Edición de Actividad Económica</h1>
      ...
      <div className="card" style={{ padding: '2rem' }}>
        ...
        <form onSubmit={handleUpdate}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '3rem' }}>
            <button type="button" className="btn-secondary" onClick={() => navigate(returnUrl, { state: { tab: 'actividad', id: id } })}>Cancelar</button>
