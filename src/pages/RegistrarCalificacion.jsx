...
export default function RegistrarCalificacion() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      await db.calificacionFinanciera.add({
        nombre: data.nombre,
        descripcion: data.descripcion
      });
      navigate('/parametros', { state: { tab: 'calificacion' } });
    } catch (err) {
      console.error(err);
      ...
      <div className="card" style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <form onSubmit={handleSave}>
          <div className="form-group" style={{ textAlign: 'left', marginBottom: '1.5rem' }}>
            ...
            <input type="text" className="form-control" name="nombre" placeholder="Nombre Calificación Financiera" style={{ fontSize: '1rem' }} required />
          </div>
          ...
          <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '0.75rem' }}>
            Registrar Calificación Financiera
          </button>
        </form>
      </div>
    </div>
