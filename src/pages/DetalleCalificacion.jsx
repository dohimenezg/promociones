...
export default function DetalleCalificacion() {
  ...
  const handleDelete = async () => {
    try {
      ...
      navigate('/parametros', { state: { tab: 'score' } });
    } catch (err) {
      console.error(err);
  ...
  const item = useLiveQuery(() => id ? db.calificacionFinanciera.get(id) : null, [id]);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '2rem' }}>
      <BackButton fallbackPath='/parametros' fallbackState={{ tab: 'calificacion' }} title="Detalle de Calificación Financiera" />
      ...
      <div className="card" style={{ padding: '2rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '3rem', gap: '1rem' }}>
          ...
          <button className="btn-secondary" style={{ display: 'flex', gap: '0.5rem', background: '#212B33' }} onClick={() => navigate('/parametros/editar-calificacion', { state: { from: '/parametros/detalle-calificacion', id: id } })}>
